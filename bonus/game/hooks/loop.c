...
/*   Updated: 2025/09/11 14:51:23 by hle-hena         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "cub3D_bonus.h"

/* Previous code for the collision. Works only on square collision.
...
void	move_event(t_data *data)
{
	...
	if (len > 0.001f)
	{
		data->map->player.x += (delta.x / len) * MOVE_SPEED * data->delta_t;
		data->map->player.y += (delta.y / len) * MOVE_SPEED * data->delta_t;
	}
}
...
int	event_loop(t_data *data)
{
	...
	mlx_put_image_to_window(data->mlx, data->win,
		data->img[data->img_buffer].img, 0, 0);
	data->img_buffer++;
