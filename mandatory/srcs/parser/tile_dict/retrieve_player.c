...
/*   Updated: 2025/08/29 11:33:43 by hle-hena         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
twice.", 0));
	if (ft_strlen(line) > 3)
		return (*err = 1, ft_perror(-1, ft_strsjoin((char *[]){"The line '",
					line, "' should be P=[char to replace the player with]. Got\
 too much char.", NULL}), 1));
	if (!line[2])
		return (*err = 1, ft_perror(-1, ft_strsjoin((char *[]){"The line '",
					line, "' is missing the char to replace the player with.",
					NULL}), 1));
