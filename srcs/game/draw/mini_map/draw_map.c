...
/*   Updated: 2025/08/28 12:38:35 by hle-hena         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	draw_tiles(t_data *data)
{
	...
	while (++curr.y < data->map->len)
	{
		...
		while (++curr.x < data->map->wid)
		{
			...
			else if (*(data->map->matrix + curr.y * data->map->wid + curr.x)
				!= data->map->void_char)
				draw_tile(data, start, vals,
					*(int *)(get_tile_dict()[*(data->map->matrix + curr.y
							* data->map->wid + curr.x)]->tex_fl->data));
		}
	}
}
