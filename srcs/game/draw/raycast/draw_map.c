...
/*   Updated: 2025/04/18 11:38:47 by hle-hena         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void draw_wall(t_data *data, t_hit ray_hit, t_point limit, int x)
{
	...
	stripe = ray_hit.stripe.data + (x % STRIPE_LEN * ray_hit.stripe.bpp) + y * ray_hit.stripe.size_line;
...
void cast_ray(t_data *data, t_vec ray_dir, t_cam cam, int x)
{
	t_hit	ray_hit;
	int		line_height;
	int		draw_start;
	...
	ray_hit.dist = ray_hit.dist * (ray_dir.x * cam.dir.x + ray_dir.y * cam.dir.y);;
