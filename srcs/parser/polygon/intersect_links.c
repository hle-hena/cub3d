...
/*   Updated: 2025/08/13 17:31:07 by hle-hena         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	intersect_links(t_link *l1, t_link *l2, t_inter *inter, t_graph *graph)
{
	...
	if (l1->type == 0 && l2->type == 0)
	{
		...
		// if (temp.dist < inter->dist || (inter->dist == -1 && temp.dist >= 0))
		// 	*inter = (t_inter){temp.coo, (t_vec){0}, 0, temp.dist};
	}
	else if (l1->type == 0)
	{
		intersect_seg_arc(l1, l2, inter, &check_possible_seg_arc, graph);
		// temp = intersect_alseg(l1, l2, graph);
		// if (temp.dist < inter->dist || (inter->dist == -1 && temp.dist >= 0))
		// 	*inter = (t_inter){temp.coo, (t_vec){0}, 0, temp.dist};
	}
	else if (l2->type == 0)
	{
		intersect_seg_arc(l2, l1, inter, &check_possible_arc_seg, graph);
		// temp = intersect_alseg(l2, l1, graph);
		// if (temp.pos < inter->dist || (inter->dist == -1 && temp.pos >= 0))
	...
	else
	{
		...
		// if (temp.dist < inter->dist || inter->dist == -1)
		// 	*inter = (t_inter){temp.coo, (t_vec){0}, 0, temp.dist};
	}
}
