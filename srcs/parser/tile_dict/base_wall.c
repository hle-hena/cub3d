...
/*   Updated: 2025/08/12 17:24:06 by hle-hena         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	add_base_wall(t_tile *tile, int *err)
{
	if (tile->wpath)
		return ;
	if (add_wall(tile, (t_wpath){(t_vec){0, 0}, (t_vec){0, 1}, (t_vec){0},
			tile->tex_we, (t_vec){-1, 0}, tile->tex_we.reflectance, 0, 0,
			1, 0, 1}, err))
		return ;
	if (add_wall(tile, (t_wpath){(t_vec){1, 0}, (t_vec){0, 0}, (t_vec){0},
			tile->tex_no, (t_vec){0, -1}, tile->tex_no.reflectance, 0, 0,
			1, 0, 1}, err))
		return ;
	if (add_wall(tile, (t_wpath){(t_vec){0, 1}, (t_vec){1, 1}, (t_vec){0},
			tile->tex_so, (t_vec){0, 1}, tile->tex_so.reflectance, 0, 0,
			1, 0, 1}, err))
		return ;
	if (add_wall(tile, (t_wpath){(t_vec){1, 1}, (t_vec){1, 0}, (t_vec){0},
